...
#            2014   Neil Nelson
#
# Validation script for data/lang_liaison
...
if (-s "$lang/phones/word_boundary.txt") {
  ...
  while (<W>) {
    ...
    if (m/^.*liaison$/) {
      s/ liaison$//g;   @col = split(" ", $_); if (@col == 1) {$liaison   .= "$col[0] ";}
    }
    if (@col != 1) {
