...
if [ $# != 5 ]; then
  echo "Usage: steps/nnet2/get_egs.sh [opts] <data> <lang> <ali-dir> <nnet-dir> <exp-dir>"
  echo " e.g.: steps/nnet2/get_egs.sh data/train data/lang exp/tri3_ali exp/nnet2 exp/tri4_nnet"
  echo ""
  ...
  echo "                                                   # process."
  echo "  --use-gpu					   # forwarding on GPU."
...
# Apply nnet forward pass to the train and validation data
feats="ark,s,cs:nnet-am-compute --use-gpu=$use_gpu $nndir/final.mdl \\\"$feats\\\" ark:- |"
valid_feats="ark,s,cs:nnet-am-compute --use-gpu=$use_gpu $nndir/final.mdl \\\"$valid_feats\\\" ark:- |"
train_subset_feats="ark,s,cs:nnet-am-compute --use-gpu=$use_gpu $nndir/final.mdl \\\"$train_subset_feats\\\" ark:- |"

echo $(hmm-info --print-args=false $alidir/final.mdl | grep pdfs | awk '{ print $NF }') > $dir/feat_dim
