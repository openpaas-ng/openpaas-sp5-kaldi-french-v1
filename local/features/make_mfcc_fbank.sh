...
mfccdir=`perl -e '($dir,$pwd)= @ARGV; if($dir!~m:^/:) { $dir = "$pwd/$dir"; } print $dir; ' $mfcc_fbank_dir ${PWD}`

# use "name" as part of name of the archive.
name=`basename $data`

mkdir -p $mfcc_fbank_dir || exit 1;
...
required="$scp $mfcc_config $fbank_config"
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_fbank_${name}.JOB.log \
    paste-feats --length-tolerance=$paste_length_tolerance "$mfcc_feats" "$fbank_feats" ark:- \| \
    copy-feats --compress=$compress ark:- \
      ark,scp:$mfcc_fbank_dir/raw_mfcc_fbank_$name.JOB.ark,$mfcc_fbank_dir/raw_mfcc_fbank_$name.JOB.scp \
     || exit 1;
...
for n in $(seq $nj); do
  cat $mfcc_fbank_dir/raw_mfcc_fbank_$name.$n.scp || exit 1;
done > $data/feats.scp
